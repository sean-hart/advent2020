...
from libs.checksum.checksum import check_survey, check_survey_all

challenge_input = """lznroxbqymvfijpwkec
...
def test_check_survey():
    for tc in testcases:
        actual = check_survey(tc['input'])
        actual2 = check_survey_all(tc['input'])
        assert tc['expected'] == actual
        assert tc['expected2'] == actual2
