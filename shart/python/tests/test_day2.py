...
from libs.password_check.password_check import (
    validate_password_for_count,
    validate_passwords_for_count,
    validate_passwords_for_position,
)

challenge_input = """9-11 p: pppppppppxblp
2-4 b: bbxbb
3-5 q: dqfqb
5-8 g: ggcgggglg
...
validate_testcases = [
    {
        'description': "has 1 to 3 'a's",
        'input': '1-3 a: abcde',
        'expected': True,
    },
    {
        'description': "has 1 to 3 'b's",
        'input': '1-3 b: cdefg',
        'expected': False,
    },
    {
        'description': "has 2 to 9 'c's",
        'input': '2-9 c: ccccccccc',
        'expected': True
    },
]

validate_multiple_testcases = [
    {
        'description': "Has 1 valid",
        'input': '2-9 c: ccccccccc',
        'expected_count': 1,
        'expected_position': 0,
    },
...
def test_validate_password():
    for tc in validate_testcases:
        output = validate_password_for_count(tc['input'])
        assert tc['expected'] == output

def test_validate_passwords():
    for tc in validate_multiple_testcases:
        output = validate_passwords_for_count(tc['input'])
        assert tc['expected_count'] == output

def test_validate_passwords_for_position():
    for tc in validate_multiple_testcases:
        output = validate_passwords_for_position(tc['input'])
        assert tc['expected_position'] == output
