...
from libs.passport_validator.passport_validator import validate_passports, validate_passports2
...
testcases = [
    {
        'description': 'Single Passport legit',
        'input': """
...
def test_validate_passports():
    for tc in testcases:
        actual = validate_passports(tc['input'])
...
def test_validate_passports():
    for tc in testcases2:
        actual = validate_passports2(tc['input'])
        assert tc['expected'] == actual
